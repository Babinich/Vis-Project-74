...
def add_jitter(series):
    noise = np.random.normal(0, jitter_amount, len(series))
    return series + noise


TEAM_DATA = pd.read_csv('Data/team_data.csv', delimiter=',')
df = TEAM_DATA
df_pca = TEAM_DATA.copy()
numerical_columns = df.select_dtypes(include='number').columns
for column in numerical_columns:
    duplicate_mask = df.duplicated(subset=column, keep=False)
    df.loc[duplicate_mask, column] = add_jitter(df.loc[duplicate_mask, column])
...
def render(app: Dash):
    ...
    def update_scatter_plot(x_axis: str, y_axis: str, filter: int, sv_clicked_data, teams: list[str],
                            clicked) -> Figure:
        ...
        # creation of graphs based on the selected filter
        if filter == 1:
            ...
            # making every point on the graph of size 10
                ...
                filter <= 6):  # creates filters for ("pld_round_of_16", "won_round_of_16", "Round of 16"),
            #                     ("pld_quarter_finals", "won_quarter_finals", "Quarter Finals"),
            #                     ("pld_semi_finals", "won_semi_finals", "Semi Finals"),
            ...
            for element in points_per_trace:
                fig.update_traces(marker=dict(size=[10 for _ in range(element[1])]),
                                  selector=dict(name=element[0]))
        ...
        # this dictionary is going to store the intial value of the dummy size list, based on the selected team in
        # search bar format {name of the trace: dummy size list, list of indexes that need to be changed (indexes are
        ...
        if teams is None:
            ...
            for team in teams:
                ...
                if label not in update_size_call.keys() and type(fig_dict["data"][indx_trace]["marker"]["size"]) == int:
                    update_size_call[label] = [[fig_dict["data"][indx_trace]["marker"]["size"]],
                                               [indx_team]]  # case distinction
                    # for team filter

                elif label not in update_size_call.keys() and type(
                        fig_dict["data"][indx_trace]["marker"]["size"]) == list:
